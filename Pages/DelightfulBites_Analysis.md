...
# Group by cuisine and restaurant, count the number of occurrences, and sort in descending order of count
restaurant_counts = df.groupby(["cuisine_type", "restaurant_name"]).size().reset_index(name="count")
restaurant_counts = restaurant_counts.sort_values(["cuisine_type", "count"], ascending=[True, False])

# Highlight the top 3 restaurants for each cuisine
restaurant_counts["highlight"] = restaurant_counts.groupby("cuisine_type")["count"].rank(method="dense", ascending=False) <= 3

# Create a bar chart for each cuisine type
for cuisine_type, group in restaurant_counts.groupby("cuisine_type"):
    # Limit to the top 10 restaurants for this cuisine type
    top_restaurants = group.nlargest(10, "count")
    
    # Filter the top 3 restaurants for this group
    top_restaurants = top_restaurants.loc[top_restaurants["highlight"]]
    
    # Create the bar chart
    plt.bar(top_restaurants["restaurant_name"], top_restaurants["count"])
    plt.xticks(rotation=90)
    plt.xlabel("Restaurant Name")
    plt.ylabel("Number of Orders")
    plt.title(f"Top 10 Most Popular Restaurants for {cuisine_type.capitalize()} Cuisine")
    plt.show()
